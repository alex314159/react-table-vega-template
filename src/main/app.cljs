...
(defn some-component []
      [:div
       ...
       [:p "You can extract the filter to reuse it. It's currently: " (str (js->clj @table/table-filter)) ". It can slow large tables as we re-render on each keystroke."]
       ...
       [:p "Filtering is more of an issue in that case. We filter the source table directly in CLJS which is slow."]
       [:p "Note we also add a dummy first column - this will be useful to show the first line as the sum of what's selected. See code for special pivot expand behaviour."]
